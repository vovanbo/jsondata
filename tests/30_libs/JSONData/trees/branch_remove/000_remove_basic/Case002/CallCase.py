...
class CallUnits(unittest.TestCase):
    ...
    def testCase100(self):
        """Remove the whole document, despite the top node.
        """
        global configdata
        global appname

        target = configdata.data
        ret = configdata.branch_remove(target['phoneNumber'][0], None)
        assert ret == True

        conf_dat = repr(configdata.data)
        conf_dat = """{u'phoneNumber': [{}, {u'type': u'office', u'number': u'313 444-555'}, {u'type': u'mobile', u'number': u'777 666-555'}], u'address': {u'city': u'New York', u'streetAddress': u'21 2nd Street', u'houseNumber': 12}}"""
    ...
    def testCase101(self):
        ...
        target = configdata.data
        ret = configdata.branch_remove(target['phoneNumber'][1], None)
        assert ret == True

        conf_dat = repr(configdata.data)
        conf_dat = """{u'phoneNumber': [{}, {}, {u'type': u'mobile', u'number': u'777 666-555'}], u'address': {u'city': u'New York', u'streetAddress': u'21 2nd Street', u'houseNumber': 12}}"""
        assert repr(configdata.data) == conf_dat
